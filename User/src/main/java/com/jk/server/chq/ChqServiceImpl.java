...
public class ChqServiceImpl implements ChqService {
   ...
   //找到本店的所有菜品
    ...
    @Override
    public void add(String area, Integer phoneNumber) {
        chqMapper.add(area,phoneNumber);
    }

    @Override
    public List<address> queryUserArea(Integer phoneNumber) {
        List<address> list =chqMapper.queryUserArea(phoneNumber);
        return list;
    }

    @Override
    public void addOrder(long dateTime, Integer userid, String storeid, String price, String msg, String area, String ids, String numbers) {
        chqMapper.addOrder(dateTime,userid,storeid,price,msg,area);
        String[] split = ids.split(",");
        String[] count = numbers.split(",");
        for (int i = 0; i<split.length;i++){
            chqMapper.addOm(dateTime,split[i],count[i]);
        }

    }

    @Override
    public menuBean chakanxiangqing(Integer id, Integer storeid) {
        return chqMapper.chakanxiangqing(id,storeid);
    }
}
